    ...
    const octokit = new Octokit({ auth: process.env.GITHUB_TOKEN });
      ...
      return {
        ...
        stats: {
          ...
          streak: {
            current: contributionData.status === 'fulfilled' ? contributionData.value.currentStreak : 0,
            longest: contributionData.status === 'fulfilled' ? contributionData.value.longestStreak : 0
          }
  ...
  async function getContributionDataWithGraphQL(username) {
    ...
    try {
      ...
      return {
        ...
        // Removed weeks to keep only calculated data
      };
    } catch (error) {
